...
dustbin_detection_model = YOLO("./computer-vision/fire-detection/models/dustbin_best.pt")
...
def send_telegram_location(latitude: float, longitude: float) -> None:
    if telegram_bot:
        telegram_bot.send_location(TELEGRAM_BOT_CHAT_ID, latitude=latitude, longitude=longitude)
...
def process_fire_detection(base64_image: str, location: dict) -> Tuple[str, int]:
    ...
    results_fire = fire_detection_model.predict(
        source=os.path.join(UPLOAD_DIRECTORY, filename), save=True, conf=0.5
    )
    
    # Run dustbin detection
    results_dustbin = dustbin_detection_model.predict(
        source=os.path.join(UPLOAD_DIRECTORY, filename), save=True
    )
    ...
    if result_dustbin:
        send_telegram_message(f"A dustbin fire has been detected at {user_location.address}")
        send_telegram_location(latitude, longitude)
        return "Dustbin fire has been detected, emergency services have been contacted. Please stay clear.", 200
    elif result_fire:
        send_telegram_message(f"A fire has been detected at {user_location.address}")
        send_telegram_location(latitude, longitude)
        return "Fire has been detected, emergency services have been contacted. Please move to a safe distance.", 200
    else:
